...
class BlogFeedAll(BaseSheetFeed):
    def feed_id(self):
    ...
    def items(self):
        return Sheet.objects.order_by("-created_at")[:ITEMS_PER_FEED]

class BlogFeedUser(BaseSheetFeed):
    ...
    def feed_updated(self, user):
        qs = Sheet.objects.filter(author=user)
        # We return an arbitrary date if there are no results, because there
        # must be a feed_updated field as per the Atom specifications, however
        # there is no real data to go by, and an arbitrary date can be static.
        if qs.count() == 0:
    ...
    def items(self, user):
        return Sheet.objects.filter(author=user).order_by("-created_at")[:ITEMS_PER_FEED]
